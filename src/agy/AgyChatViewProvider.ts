...
export class AgyChatViewProvider implements vscode.WebviewViewProvider {
    ...
    /** Last non-empty editor selection. The webview steals focus, so asking
     *  activeTextEditor at click time is too late; remember it as it changes. */
    private editorSelection?: EditorSelectionContext;

    constructor(private readonly context: vscode.ExtensionContext) {
        context.subscriptions.push(
            vscode.window.onDidChangeTextEditorSelection((e) => this.refreshEditorSelection(e.textEditor)),
            vscode.window.onDidChangeActiveTextEditor((editor) => this.refreshEditorSelection(editor)),
        );
        this.refreshEditorSelection(vscode.window.activeTextEditor);
    }
    ...
    /**
     * Fix a stored model id that agy rejects, in place.
     *
     * The value usually lost its vendor prefix in an older build, so the fix is
     * to find the real id whose prefix-stripped form matches and switch to it —
     * "opus-4-6-thinking" becomes "claude-opus-4-6-thinking", which works. Runs
     * `agy models` for the authoritative list. Falls back to the default only
     * when no id matches at all.
     */
    private repairModel(bad: string): void {
        ...
        if (!cli) {
            giveUp('The selected model is not available; reset to the default. Send again to use it.');
            return;
        }
        ...
        child.on('error', () => giveUp('The selected model is not available; reset to the default.'));
        child.on('close', () => {
            const ids = out.split(/\r?\n/).map((l) => l.trim()).filter((l) => l && !/\s/.test(l));
            const strip = (m: string) => m.replace(/^(gemini|claude|gpt|gpt-oss)-/, '');
            // Exact first, then prefix-stripped, then a suffix match as a last try.
            const match = ids.find((m) => m === bad)
                || ids.find((m) => strip(m) === bad)
                || ids.find((m) => strip(m) === strip(bad))
                || ids.find((m) => m.endsWith('-' + bad) || m.endsWith(bad));
            if (match) {
                this.context.globalState.update('antigravity.model', match);
                this.post({ type: 'models', models: ids, current: match });
                this.post({
                    type: 'error',
                    text: 'Updated the saved model id to "' + match + '". Send again — it will work now.',
                });
            } else {
                this.context.globalState.update('antigravity.model', '');
                this.post({ type: 'models', models: ids, current: '' });
                this.post({
                    type: 'error',
                    text: 'That model is not one agy offers; reset to the default. Send again, or ' +
                        'pick another from the model chip.',
                });
            }
        });
    }
    ...
    private refreshEditorSelection(editor: vscode.TextEditor | undefined): void {
        // Focusing the side panel can leave activeTextEditor undefined. Keep
        // the last real editor selection so the composer chip survives the
        // click from editor to panel.
        if (!editor) {
            this.postSelection();
            return;
        }
        this.editorSelection = this.selectionContext(editor);
        this.postSelection();
    }

    private selectionContext(editor: vscode.TextEditor | undefined): EditorSelectionContext | undefined {
        ...
        const blocks = editor.selections
            ...
            .map((selection) => {
                ...
                return {
                    ...
                    endLine: selection.start.line + Math.max(1, lineCount),
                    lineCount,
                    text,
                };
            })
        ...
        const file = rel !== editor.document.uri.fsPath ? rel : editor.document.fileName;
        return {
            file,
    ...
    private formatEditorSelection(selection: EditorSelectionContext): string {
        const blockWord = selection.blockCount === 1 ? 'selection' : 'selections';
        const lineWord = selection.lineCount === 1 ? 'line' : 'lines';
        const header = selection.blockCount === 1
            ? `Selected text from ${selection.file} (${selection.lineCount} ${lineWord}):`
            : `Selected text from ${selection.file} (${selection.blockCount} ${blockWord}, ` +
                `${selection.lineCount} ${lineWord} total):`;
        const fenced = selection.blocks.map((block) => {
            const lines = block.startLine === block.endLine
    ...
    private attachEditorSelection(): void {
        const fresh = this.selectionContext(vscode.window.activeTextEditor);
        if (fresh) {
            this.editorSelection = fresh;
            this.postSelection();
        }
        if (!this.editorSelection) {
            this.post({ type: 'selectionMissing' });
            return;
        }
        this.post({ type: 'selectionAttached', text: this.formatEditorSelection(this.editorSelection) });
    }
        ...
        if (ev.event === 'result') {
            ...
            if (String(r.status).toUpperCase() === 'ERROR' && r.error) {
                ...
                if (/model .* is not recognized|invalid model/i.test(err) && this.model) {
                    // Repair, don't just discard. The bad value is almost always
                    // a real model with its vendor prefix dropped by an old build
                    // ("opus-4-6-thinking" for "claude-opus-4-6-thinking"). Match
                    // it against the live list and migrate to the full id, so the
                    // user keeps the model they chose instead of dropping to
                    // Default. Only if nothing matches do we reset.
                    this.repairModel(this.model);
                } else {
    ...
    private run(prompt: string): void {
        ...
        // agy's print mode gives up after 5 minutes by default and reports
        // "timeout waiting for response". An agentic turn — browser tests, a
        // build, a dev server — routinely runs longer, so raise the ceiling.
        // Configurable, and 0/blank leaves agy on its own default.
        const printTimeout = vscode.workspace
            .getConfiguration('antigravity')
            .get<string>('printTimeout', '30m')
            .trim();
        if (printTimeout && printTimeout !== '0') {
        ...
        child.on('close', (code) => {
            ...
            if (denied && !produced) {
                ...
                const timedOut = /timeout waiting for response/i.test(lastErr);
                this.post({
                    type: 'error',
                    text: timedOut
                        ? 'The turn ran past its time limit before agy answered. A command it ' +
                          'started may not have exited (a dev server, a watch). Raise ' +
                          'antigravity.printTimeout, or make the command exit on its own.'
                        : (lastErr || `agy exited with code ${code}.`),
                });
            } else if (!produced) {
                this.post({
                    type: 'error',
    ...
    private html(webview: vscode.Webview): string {
        ...
        const author = String(pkg.author?.name || pkg.publisher || '');
        const cache = version ? `v=${encodeURIComponent(version)}` : '';
        const css = webview.asWebviewUri(vscode.Uri.joinPath(media, 'panel.css')).with({ query: cache });
        const js = webview.asWebviewUri(vscode.Uri.joinPath(media, 'panel.js')).with({ query: cache });
        const mdjs = webview.asWebviewUri(vscode.Uri.joinPath(media, 'markdown.js')).with({ query: cache });
        const n = this.nonce();
        const csp =
...
</head><body data-extension-version="${this.escAttr(version)}" data-extension-author="${this.escAttr(author)}">
<div class="statusbar">
...
<form id="composer" class="composer">
  <div class="inputwrap">
    ...
    <div class="tools">
      ...
      <button id="selectionBtn" class="selectionchip" type="button" title="Attach selected text (Ctrl+Esc)" aria-label="Attach selected text" hidden>
        <svg viewBox="0 0 16 16" class="ico" aria-hidden="true"><path d="M9 2.4H4.6a1 1 0 0 0-1 1v9.2a1 1 0 0 0 1 1h6.8a1 1 0 0 0 1-1V5.8z" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/><path d="M9 2.4v3.4h3.4" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/></svg>
        <span id="selectionText"></span></button>
      <span class="grow"></span>
