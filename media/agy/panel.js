...
(function () {
  ...
  let extVersion = document.body.dataset.extensionVersion || "";
  let extAuthor = document.body.dataset.extensionAuthor || "";
  ...
  function showEmpty() {
    ...
    log.innerHTML =
      ...
      (extVersion || extAuthor
        ...
        : "") +
      (connState === "ok"
        ? '<a href="#" class="empty-star" data-act="star">★ Star on GitHub if it helps</a>'
        : "") +
      "</div>";
  }
  ...
  log.addEventListener("click", (e) => {
    ...
    if (e.target.closest('[data-act="star"]')) {
      e.preventDefault();
      vscode.postMessage({ type: "openRepo" });
      return;
    }
  ...
  function selectionLabel(s) {
    if (!s || !s.lineCount) return "";
    if (s.blockCount && s.blockCount > 1) {
      return s.blockCount + " selections · " + s.lineCount + " lines";
    }
    return s.lineCount + " " + (s.lineCount === 1 ? "line" : "lines") + " selected";
  }

  function paintPlaceholder() {
    input.placeholder = busy
      ? "Queue a follow-up…  •  Esc to stop"
      : (editorSelection ? "Ctrl+Esc to attach selected text" : IDLE_PLACEHOLDER);
  }

  function paintEditorSelection() {
    if (!selectionBtn || !selectionText) return;
    const label = selectionLabel(editorSelection);
    selectionBtn.hidden = !label;
    selectionText.textContent = label;
    if (editorSelection) {
      selectionBtn.title = "Attach selected text from " + editorSelection.file + " (Ctrl+Esc)";
      selectionBtn.setAttribute("aria-label", "Attach " + label);
    } else {
      selectionBtn.title = "Attach selected text (Ctrl+Esc)";
      selectionBtn.setAttribute("aria-label", "Attach selected text");
    }
    paintPlaceholder();
  }
  ...
  function submit() {
    ...
    if (busy) {
      // Do not queue the same line twice in a row. A hung turn invites
      // mashing Enter, and six identical copies were turning into a run-on
      // blob when the turn finally failed.
      if (queued[queued.length - 1] !== text) {
        queued.push(text);
        paintQueue();
      }
      input.value = "";
  ...
  input.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && e.ctrlKey) {
      if (attachEditorSelection()) e.preventDefault();
      return;
    }
  ...
  window.addEventListener("message", (event) => {
    ...
    switch (msg.type) {
      case "selection":
        editorSelection = msg.selection || null;
        paintEditorSelection();
        break;
      case "selectionAttached":
        appendToInput(msg.text || "");
        break;
      case "selectionMissing":
        addTurn("error", "No editor text is selected.");
        break;
      case "installing": {
        connState = "connecting";
        connDetail = msg.text || "";
  ...
  function describeModel(id) {
    ...
    // Slug to a name: join a dash-written version (4-6 -> 4.6), title-case
    // words (flash -> Flash), uppercase acronyms and size letters (oss -> OSS,
    // 120b -> 120B).
    const ACR = { oss: "OSS" };
    const rest = base
      .replace(/^(gemini|claude|gpt)-/, "")
      .replace(/(\d)-(\d)/g, "$1.$2")
      .split("-")
      .map((w) => ACR[w] || (/\d/.test(w)
        ? w.toUpperCase()
