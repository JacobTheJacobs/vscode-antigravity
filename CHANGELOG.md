...
- Removed the cross-promotion section from the README.

## 4.1.2

- The empty-state byline now receives the extension version from the package
  metadata before the webview connects, and webview assets are cache-busted by
  version so updates do not keep showing stale UI.

## 4.1.1

- Rebuilt the VSIX for local VS Code installation.

## 4.1.0

- **Selected code appears in the composer.** Select lines in VS Code, then
  click the bottom chip or press **Ctrl+Esc** to attach the selection to the
  prompt with file name, line range and language fence.
- Replaced the README demo GIF with a model-switching and file-reading flow.
- Added project community files: contributing guide, security policy, support
  notes, code of conduct, notices, issue templates and pull request template.
- Fixed CI/release workflows to match the package scripts and publish the
  correct `antigravity-chat` VSIX name.

## 4.0.0

- Repository history reset to a single `init` commit.
- README and package metadata now point at
  `JacobTheJacobs/vscode-antigravity`.
- License notices preserved under MIT.
